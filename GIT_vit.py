...
class Attention(Module):# multi-head self attention
...
class Transformer(Module):# encoder
...
class ViT(Module):
    ...
    def forward(self, img):
        ...
        x = x.mean(dim = 1) if self.pool == 'mean' else x[:, 0] # 提取 cls token 或者使用平均池化
